      ...
      for (var routine in routinesToAdd) {
        if (routine.isAiGenerated) {
          if (routine.parts.isNotEmpty) {
            // Routine already generated by AI in the UI path; just enrich weights and save
            final enriched = await _aiRoutineGenerationService
                .enrichRoutineWithRecommendedWeights(routine, userProfile: userProfile);
            final newId = await _dbProvider.newRoutine(enriched);
            routinesWithIds.add(enriched.copyWith(id: newId));
          } else {
            // Placeholder AI routine request: generate via service
