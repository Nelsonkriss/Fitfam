  ...
  Widget _buildMoves() {
    ...
    if (widget.exNames.isNotEmpty) {
      for (var exName in widget.exNames) {
        children..add(_buildRow(exName))..add(const Divider());
      }
...
class ExerciseNameListView extends StatefulWidget {
  ...
  const ExerciseNameListView({Key? key, required this.exNames}) : super(key: key);

  @override
  ExerciseNameListViewState createState() => ExerciseNameListViewState();
}